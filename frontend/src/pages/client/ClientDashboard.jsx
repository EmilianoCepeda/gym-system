...
const ClientDashboard = () => {
  ...
  const fetchAll = async () => {
    try {
      ...
      // Get subscription from user subscriptions endpoint
      try {
  ...
  const navItems = [
    ...
    { id: 'routines',  label: 'Mis Rutinas',   icon: <Dumbbell size={20} /> },
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-base)', fontFamily: 'var(--font-body)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                ...
                <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', padding: '28px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px' }}>
                    <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: 'var(--yellow-dim)', border: '2px solid var(--yellow)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <span style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: '24px', color: 'var(--yellow)' }}>{user?.name?.[0]?.toUpperCase()}</span>
                    </div>
                    <div>
                      <p style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '20px', margin: 0, letterSpacing: '1px' }}>{user?.name}</p>
                      <p style={{ fontSize: '13px', color: 'var(--text-muted)', margin: '4px 0 0' }}>{user?.email}</p>
                    </div>
                  </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {[
                          ...
                          { label: 'Vence', value: new Date(subscription.endDate).toLocaleDateString('es-MX', { timeZone: 'America/Monterrey' }) },
                          {
                            ...
                            value: <span style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '18px', color: daysColor(daysLeft(subscription.endDate)) }}>{daysLeft(subscription.endDate)}d</span>
                          },
                ...
                <div style={{ marginTop: '40px' }}>
                  ...
                  <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
                    {myReservations.map(r => (
                      <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
                        <div>
                          <p style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '16px', margin: 0, letterSpacing: '1px', textTransform: 'uppercase' }}>{r.class.name}</p>
                          <p style={{ fontSize: '12px', color: 'var(--text-muted)', margin: '4px 0 0' }}>
                            {new Date(r.class.date).toLocaleDateString('es-MX', { timeZone: 'America/Monterrey' })} · Coach: {r.class.coach.name}
                          </p>
                        </div>
                        <span className={`badge ${r.status === 'ACTIVE' ? 'badge-green' : 'badge-gray'}`}>{r.status === 'ACTIVE' ? 'Activa' : 'Cancelada'}</span>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  {routines.map(r => (
                    <div key={r.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', padding: '28px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                        ...
                        <span style={{ fontSize: '11px', color: 'var(--text-subtle)', letterSpacing: '2px', textTransform: 'uppercase' }}>
          ...
          {activeTab === 'progress' && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                ...
                <button className="btn-primary" style={{ padding: '10px 24px', fontSize: '14px', display: 'flex', alignItems: 'center', gap: '8px' }} onClick={() => setShowProgressModal(true)}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                {[...progress].reverse().map(p => (
                  <div key={p.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button onClick={() => handleDeleteProgress(p.id)} style={{ background: 'none', border: 'none', color: 'var(--text-subtle)', cursor: 'pointer', transition: 'color 0.2s' }}
                      onMouseEnter={e => e.currentTarget.style.color = '#ff4444'}
                      onMouseLeave={e => e.currentTarget.style.color = 'var(--text-subtle)'}
                    >
                      <Trash2 size={16} />
              ...
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', padding: '32px' }}>
                <form onSubmit={handleSaveProfile} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  ...
                  <button
                    ...
                    className="btn-primary"
                    style={{ width: '100%', textAlign: 'center', opacity: savingProfile ? 0.6 : 1, cursor: savingProfile ? 'not-allowed' : 'pointer' }}
                  >
      ...
      {showProgressModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', padding: '40px', width: '500px', maxWidth: '90vw', maxHeight: '90vh', overflowY: 'auto' }}>
